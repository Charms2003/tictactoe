...
const huPlayer = 'X';
const aiPlayer = 'O';
const countX = document.querySelector("#scoreX");
const countO = document.querySelector("#scoreO");
...
function updateTurnInfo() {
    if (turnInfoElement) {
        turnInfoElement.innerText = `Turn: Player ${currentPlayer === huPlayer ? 'X' : 'O'}`;
    }
}
...
function turnClick(square) {
    if (typeof origBoard[square.target.id] === 'number') {
        ...
        updateTurnInfo(aiPlayer);  // Update turn info for AI's turn
        ...
        if (!checkWin(origBoard, huPlayer) && !checkTie()) {
            setTimeout(() => {
                ...
                updateTurnInfo(huPlayer);  // Update turn info for human's turn
            }, 200);
        }
    }
}
...
function gameOver(gameWon) {
    ...
    declareWinner(gameWon.player == huPlayer ? "You Earned a Point!" : "AI Earned a Point.");
...
function bestSpot() {
    // Specify the win conditions to block
    const winConditionsToBlock = [
        [4, 11],
        [1, 6],
        [18, 25],
        [23, 28]
        // Add more win conditions to block as needed
    ];

    // Filter available spots to include only those in win conditions to block
    const blockingSpots = emptySquares().filter(move => canBlockWinCondition(move, winConditionsToBlock));

    // If there are blocking spots, randomly choose one; otherwise, move randomly
    return blockingSpots.length > 0 ? randomMoveFromArray(blockingSpots) : randomMove();
}

function canBlockWinCondition(move, winConditionsToBlock) {
    for (let combo of winConditionsToBlock) {
...
function randomMoveFromArray(array) {
...
function restartButton() {
    ...
    winCheckElement.innerText = "";
    resetScores();
    startGame(); // Restart the game without reloading the page
}
